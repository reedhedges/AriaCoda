...
SimpleMod module1;
ARDEF_MODULE(module1);

bool SimpleMod::init(ArRobot *robot, void *argument)
{
  ArLog::log(ArLog::Normal, "moduleActionExample_Mod: Robot name is %s", robot->getRobotName());
  ArLog::log(ArLog::Terse, "moduleActionExample_Mod: init(%p) called in the loaded module!", robot);
  if (argument != NULL)
    ArLog::log(ArLog::Terse, "moduleActionExample_Mod: Argument given to ArModuleLoader::load was the string '%s'.", 
	   (char *)argument);
  else
    ArLog::log(ArLog::Terse, "moduleActionExample_Mod: No argument was given to ArModuleLoader (this is OK).");
    
  // Do stuff here...
  
  robot->addAction(&myConstantVelocity, 50);
  myConstantVelocity.activate();
...
bool SimpleMod::exit()
{
  ArLog::log(ArLog::Terse, "moduleActionExample_Mod: exit() called.");
