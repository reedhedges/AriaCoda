...
RobotRadius 600.00000    ; radius in mm
RobotDiagonal 600.00000  ; half-height to diagonal of octagon
RobotWidth 830.00000     ; width in mm
RobotLength 1200.00000   ; length in mm of the whole robot
RobotLengthFront 600.00000 ; length in mm to the front of the robot (if this is
                         ; 0 (or non existent) this value will be set to half of
                         ; RobotLength)
RobotLengthRear 600.00000 ; length in mm to the rear of the robot (if this is 0
...
SonarNum 0               ; Number of sonars on the robot.
...
LaserAutoConnect true    ; Laser_1 exists and should be automatically connected
                         ; at startup.
LaserX 527               ; Location (in mm) of the laser in X (+ front, - back)
...
LaserCumulativeBufferSize 0 ; Cumulative buffer size to use for the laser. 0 to
                         ; use the default.
LaserStartDegrees -117   ; Start angle (in deg) for the laser. This may be used
                         ; to constrain the angle. Fractional degrees are
                         ; permitted. Leave blank to use the default.
LaserEndDegrees 117      ; End angle (in deg) for the laser. This may be used to
...
PTZType rvision          ; PTZ or PTU type
PTZInverted false        ; If unit is mounted inverted (upside-down)
PTZSerialPort COM4       ; serial port, or none if not using serial port
                         ; communication
