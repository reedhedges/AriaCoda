...
Class MTX                ; general type of robot
Subclass pioneer-lx-ld   ; specific type of robot
RobotRadius 348.00000    ; radius in mm
RobotDiagonal 120.00000  ; half-height to diagonal of octagon
RobotWidth 500.00000     ; width in mm
RobotLength 696.00000    ; length in mm of the whole robot
RobotLengthFront 348.00000 ; length in mm to the front of the robot (if this is
                         ; 0 (or non existent) this value will be set to half of
                         ; RobotLength)
RobotLengthRear 348.00000 ; length in mm to the rear of the robot (if this is 0
                         ; (or non existent) this value will be set to half of
                         ; RobotLength)
Holonomic true           ; turns in own radius
MaxRVelocity 180         ; absolute maximum degrees / sec
MaxVelocity 2500         ; absolute maximum mm / sec
...
NumFrontBumpers 3        ; number of front bumpers on the robot
...
TransVelMax 1800         ; maximum desired translational velocity for the robot
RotVelMax 180            ; maximum desired rotational velocity for the robot
SettableAccsDecs true    ; if the accel and decel parameters can be set
TransAccel 500           ; translational acceleration
TransDecel 600           ; translational deceleration
RotAccel 150             ; rotational acceleration
RotDecel 200             ; rotational deceleration
...
SonarNum 4               ; Number of sonars on the robot.
                         ...
                         ; flag>
SonarUnit 0 331 61 10 1 1 0 600 400 0
SonarUnit 1 331 -61 -10 1 2 0 600 400 0
...
SonarAutoConnect true    ; SonarBoard_1 exists and should be automatically
                         ; connected at startup.
SonarBoardType mtx       ; Type of the sonar board.
SonarBoardPortType serial422 ; Port type that the sonar is on.
SonarBoardPort /dev/ttyUSB2 ; Port the sonar is on.
SonarBoardPowerOutput Sonar_1_Power ; Power output that controls this Sonar
                         ; Board's power.
SonarBaud 115200         ; Baud rate for the sonar board communication. (9600,
                         ; 19200, 38400, etc.).
...
LaserIgnore -135--120 120-135 ; Angles (in deg) at which to ignore readings, +/1
                         ; one degree. Angles are entered as strings, separated
                         ; by a space.
...
LaserPort /dev/ttyUSB4   ; Port the laser is on.
LaserPowerOutput         ; Power output that controls this laser's power.
LaserStartingBaudChoice 230400 ; StartingBaud for this laser. Leave blank to use
                         ; the default.
...
LaserAutoConnect true    ; Laser_2 exists and should be automatically connected
                         ; at startup.
LaserX 267               ; Location (in mm) of the laser in X (+ front, - back)
                         ; relative to the robot's idealized center of rotation.
LaserY 0                 ; Location (in mm) of the laser in Y (+ left, - right)
...
LaserTh 0.00000          ; range [-180, 180], Rotation (in deg) of the laser (+
                         ; counterclockwise, - clockwise).
LaserZ 58                ; minimum 0, Height (in mm) of the laser from the
                         ; ground. 0 means unknown.
...
LaserType tim510         ; Type of laser.
LaserPortType serial     ; Type of port the laser is on.
LaserPort /dev/ttyUSB7   ; Port the laser is on.
LaserPowerOutput         ; Power output that controls this laser's power.
...
LaserCumulativeBufferSize 0 ; Cumulative buffer size to use for the laser. 0 to
                         ; use the default.
LaserStartDegrees -61    ; Start angle (in deg) for the laser. This may be used
                         ; to constrain the angle. Fractional degrees are
                         ; permitted. Leave blank to use the default.
LaserEndDegrees 61       ; End angle (in deg) for the laser. This may be used to
                         ; constrain the angle. Fractional degreees are
                         ; permitted. Leave blank to use the default.
LaserDegreesChoice       ; Degrees choice for the laser. This may be used to
                         ; constrain the range. Leave blank to use the default.
...
BatteryAutoConnect true  ; Battery_1 exists and should be automatically
                         ; connected at startup.
BatteryType mtx          ; Type of battery.
BatteryPortType serial   ; Port type that the battery is on.
BatteryPort /dev/ttyUSB1 ; Port the battery is on.
BatteryBaud 115200       ; Baud rate to use for battery communication (9600,
                         ; 19200, 38400, etc.).

Section LCD_1
; The physical definition of this LCD.
;SectionFlags for LCD_1: 
LCDAutoConnect true      ; LCD_1 exists and should automatically be connected at
                         ; startup.
LCDDisconnectOnConnectFailure false ; The LCD is a key component and is required
                         ...
                         ; will restart.
LCDType mtx              ; Type of LCD.
LCDPortType serial422    ; Port type that the LCD is on.
LCDPort /dev/ttyUSB8     ; Port that the LCD is on.
LCDPowerOutput           ; Power output that controls this LCD's power.
LCDBaud 115200           ; Baud rate for the LCD communication (9600, 19200,
                         ; 38400, etc.).
...
VideoAddress 192.168.0.90 ; IP address or hostname, or none if not using network
                         ; communication.
